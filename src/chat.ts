...
class TokenMetricsChatInterface {
  ...
  private async fetchTokenMappings(): Promise<{ [key: string]: string }> {
    ...
    try {
      ...
      const result = await this.retryWithBackoff(async () => {
        return await this.plugin.getTokens.executable(
          { limit: "100", page: "1" }, // Get more tokens for better coverage
      ...
      if (result.status === 'done') {
        ...
        if (responseMatch) {
          ...
          if (responseData.success && responseData.data) {
            ...
            responseData.data.forEach((token: any) => {
  ...
  private async extractTokensFromPrompt(prompt: string): Promise<string[]> {
    ...
    const majorCryptoSearchTerms: { [key: string]: string[] } = {
      'bitcoin': ['bitcoin', 'btc'],
    ...
    const requestedMajorCryptos: string[] = [];
  ...
  private async testTokenDetection(): Promise<void> {
    ...
    for (const term of searchTerms) {
      const matches = Object.entries(tokenMappings).filter(([name, id]) => 
